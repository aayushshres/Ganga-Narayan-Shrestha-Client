...
import { fetchInterviews, createInterview, updateInterview, deleteInterview } from '../api/index';
...
export default function InterviewsPage() {
  ...
  const loadInterviews = () => {
    setLoading(true);
    fetchInterviews().then(setInterviews).catch(console.error).finally(() => setLoading(false));
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setTitle(''); setChannel(''); setYoutubeId('');
      loadInterviews();
  ...
  const startEdit = (i: Interview) => {
    setEditId(i._id);
    setEditData({ title: i.title, channel: i.channel, youtubeId: i.youtubeId, order: i.order });
  };
  ...
  return (
    <div>
      <h2 style={{ fontFamily: 'var(--font-display)', marginBottom: '1.5rem' }}>नयाँ अन्तर्वार्ता थप्नुहोस्</h2>
      ...
      <div style={{ background: 'var(--bg-card)', padding: '2rem', borderRadius: '8px', border: '1px solid var(--border-color)', marginBottom: '3rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          ...
          <input type="text" style={inputStyle} value={channel} onChange={(e) => setChannel(e.target.value)} placeholder="e.g. Himalaya TV" />
        </div>
        <div style={{ gridColumn: '1 / -1' }}>
      ...
      <h2 style={{ fontFamily: 'var(--font-display)', marginBottom: '1.5rem' }}>अन्तर्वार्ताहरू</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden' }}>
          <thead style={{ background: 'var(--bg-secondary)', textAlign: 'left' }}>
            <tr>
              ...
              <th style={{ padding: '1rem', borderBottom: '1px solid var(--border-color)' }}>च्यानल</th>
          ...
          <tbody>
            {interviews.map((i) => (
              editId === i._id ? (
                <tr key={i._id}>
                  <td colSpan={3} style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                      ...
                      <div style={{ gridColumn: '1 / -1' }}>
                        <label style={labelStyle}>च्यानल</label>
                        <input type="text" style={inputStyle} value={editData.channel ?? ''} onChange={(e) => setEditData({ ...editData, channel: e.target.value })} />
                      </div>
                      <div style={{ gridColumn: '1 / -1' }}>
                        <label style={labelStyle}>YouTube ID</label>
                ...
                <tr key={i._id}>
                  <td style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)' }}>{i.title}</td>
                  <td style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)' }}>{i.channel}</td>
                  <td style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)', textAlign: 'right', display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                    <button onClick={() => startEdit(i)} style={{ padding: '0.4rem 0.8rem', background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', cursor: 'pointer', fontSize: '0.9rem' }}>
                      सम्पादन गर्नुहोस्
                    </button>
                    <button onClick={() => handleDelete(i._id)} style={{ padding: '0.4rem 0.8rem', background: '#D32F2F', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.9rem' }}>
                      मेट्नुहोस्
                    </button>
                  </td>
                </tr>
            ...
            {interviews.length === 0 && (
              <tr><td colSpan={3} style={{ padding: '2rem', textAlign: 'center' }}>कुनै अन्तर्वार्ता भेटिएन।</td></tr>
            )}
          </tbody>
        </table>
      )}
