...
import { fetchArticles, createArticle, updateArticle, deleteArticle } from '../api/index';
...
export default function ArticlesPage() {
  ...
  const [editCategory, setEditCategory] = useState<'article' | 'literature'>('article');
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await updateArticle(editId, {
        title: editTitle, category: editCategory, excerpt: editExcerpt, content: editContent,
      }, token);
  ...
  return (
    <div>
      <h2 style={{ fontFamily: 'var(--font-display)', marginBottom: '1.5rem' }}>नयाँ लेख थप्नुहोस्</h2>
      ...
      <div style={{
        ...
        <div>
          ...
          <select style={inputStyle} value={category} onChange={(e) => setCategory(e.target.value as 'article' | 'literature')}>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          ...
          <textarea style={{ ...inputStyle, height: '80px', resize: 'vertical' }} value={excerpt} onChange={(e) => setExcerpt(e.target.value)} />
        </div>

        <div style={{ gridColumn: '1 / -1' }}>
          <label style={labelStyle}>सामग्री</label>
          <textarea style={{ ...inputStyle, height: '200px', resize: 'vertical' }} value={content} onChange={(e) => setContent(e.target.value)} />
        </div>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          ...
          <button onClick={handleAdd} disabled={isSubmitting || !title || !excerpt || !content} style={{
            background: 'var(--crimson)', color: 'white', border: 'none', padding: '0.75rem 2rem',
            borderRadius: '4px', cursor: (isSubmitting || !title || !excerpt || !content) ? 'not-allowed' : 'pointer',
      ...
      <h2 style={{ fontFamily: 'var(--font-display)', marginBottom: '1.5rem' }}>लेखहरू व्यवस्थापन</h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', overflow: 'hidden' }}>
          <thead style={{ background: 'var(--bg-secondary)', textAlign: 'left' }}>
            <tr>
              ...
              <th style={{ padding: '1rem', borderBottom: '1px solid var(--border-color)' }}>श्रेणी</th>
              <th style={{ padding: '1rem', borderBottom: '1px solid var(--border-color)' }}>मिति</th>
          ...
          <tbody>
            {articles.map((a) => (
              editId === a._id ? (
                <tr key={a._id}>
                  <td colSpan={4} style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                      ...
                      <div>
                        <label style={labelStyle}>श्रेणी</label>
                        <select style={inputStyle} value={editCategory} onChange={(e) => setEditCategory(e.target.value as 'article' | 'literature')}>
                          <option value="article">लेख</option>
                          <option value="literature">साहित्य</option>
                        </select>
                      </div>
                      <div style={{ gridColumn: '1 / -1' }}>
                        <label style={labelStyle}>सारांश</label>
                        <textarea style={{ ...inputStyle, height: '60px', resize: 'vertical' }} value={editExcerpt} onChange={(e) => setEditExcerpt(e.target.value)} />
                      </div>
                      <div style={{ gridColumn: '1 / -1' }}>
                        <label style={labelStyle}>सामग्री</label>
                        <textarea style={{ ...inputStyle, height: '120px', resize: 'vertical' }} value={editContent} onChange={(e) => setEditContent(e.target.value)} />
                      </div>
                ...
                <tr key={a._id}>
                  <td style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)' }}>{a.title}</td>
                  <td style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)' }}>
                    <span style={{ background: 'var(--crimson)', color: 'white', padding: '0.2rem 0.5rem', borderRadius: '4px', fontSize: '0.8rem' }}>
                  ...
                  <td style={{ padding: '1rem', borderBottom: '1px solid var(--border-light)', textAlign: 'right', display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                    <button onClick={() => startEdit(a)} style={{ padding: '0.4rem 0.8rem', background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', cursor: 'pointer', fontSize: '0.9rem' }}>
                      सम्पादन गर्नुहोस्
                    </button>
                    <button onClick={() => handleDelete(a._id)} style={{ padding: '0.4rem 0.8rem', background: '#D32F2F', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.9rem' }}>
                      मेट्नुहोस्
                    </button>
                  </td>
                </tr>
            ...
            {articles.length === 0 && (
              <tr><td colSpan={4} style={{ padding: '2rem', textAlign: 'center' }}>कुनै लेख भेटिएन।</td></tr>
            )}
          </tbody>
        </table>
      )}
